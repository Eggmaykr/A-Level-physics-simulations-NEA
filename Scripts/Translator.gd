...
func get_defaults():
	var ReturnData : String = var2str({"Data": Data})
	return ReturnData

func load_settings(settings):
	var settingsArray = str2var(settings["ItemSettings"])
	Data = settingsArray["Data"]
	Update_Data_Visual()


func update_settings():
	Database.update_database("Experiments", "ItemID", get_parent().get_parent().UniqSelfID, {"ItemSettings" :  var2str({"Data": Data})})
